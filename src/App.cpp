...
void App::loadModel()
{
    ...
    for (int i = 0; i < 5;i++) {
        for (int j = 0; j < 5; j++) {
            mesh2 = new Mesh( _device, MODEL_PATH );
            objects.push_back( { mesh2, glm::translate( glm::mat4( 1 ),glm::vec3( -5 + i * -2.5,0,j*2.5 ) ),mat2 } );
        }
    }
...
void App::createLightBuffer()
{
    ...
    Light l;
    l.color = glm::vec4( 0.2, 0.2, 1, 1 );;
    l.intensity = 1;
    l.pos_dir = glm::vec3( 0.5, -1, 1 );
    l.type = 0;
...
void App::createDescriptorSets()
{
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        const VkDescriptorImageInfo images[] = { imageInfo,imageInfo2 };
        ...
        descriptorWrites[0].pBufferInfo = VK_NULL_HANDLE;
